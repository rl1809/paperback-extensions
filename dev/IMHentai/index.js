...
class IMHentai {
            ...
            interceptor: {
                interceptRequest: async (request) => {
                    request.headers = {
                        ...(request.headers ?? {}),
                        ...{
                            referer: `${constant_1.IMHENTAI_DOMAIN}/`,
    ...
    async getMangaDetails(mangaId) {
        const $ = await this.DOMHTML(`${constant_1.IMHENTAI_DOMAIN}/gallery/${mangaId}`);
        return (0, IMHentaiParser_1.parseMangaDetails)($, mangaId);
    }
    async getChapters(mangaId) {
        const $ = await this.DOMHTML(`${constant_1.IMHENTAI_DOMAIN}/gallery/${mangaId}`);
        return (0, IMHentaiParser_1.parseChapterList)($, mangaId);
    }
    async getChapterDetails(mangaId, chapterId) {
        const $ = await this.DOMHTML(`${constant_1.IMHENTAI_DOMAIN}/gallery/${chapterId}`);
        return (0, IMHentaiParser_1.parseChapterDetails)($, mangaId, chapterId);
    }
    ...
    async getSearchTags() {
        const sections = [
            App.createTagSection({ id: '0', label: 'tags (advanced search)', tags: search_json_1.popularTags.map(x => App.createTag(x)) }),
            App.createTagSection({ id: '1', label: 'categories (multiple choice)', tags: search_json_1.categories.map(x => App.createTag(x)) }),
            App.createTagSection({ id: '2', label: 'languages (multiple choice)', tags: search_json_1.languages.map(x => App.createTag(x)) }),
            App.createTagSection({ id: '3', label: 'order by (choose 1)', tags: search_json_1.order.map(x => App.createTag(x)) }),
        ];
        return sections;
    }
...
const parseChapterDetails = ($, mangaId, chapterId) => {
    ...
    const domainSplit = constant_1.IMHENTAI_DOMAIN.split('//');
...
const parseSearch = ($, excludedTags) => {
    ...
    for (const obj of $('div.thumb', constant_1.directoryGallerySelector).toArray()) {
        const image = getImageSrc($('img', $('div.inner_thumb', obj)).first() ?? '');
...
exports.tagBoxSelector = 'div.row.stags';
