...
import { KEMONO_DOMAIN, IMG_KEMONO_DOMAIN } from "./constant";

import { services, sortBy, sortType } from "./external/search.json"

export const KemonoInfo: SourceInfo = {
    version: "0.0.1",
    name: "Kemono",
    icon: "icon.png",
    author: "Thanh Nha",
...
export class Kemono
    ...
    getMangaShareUrl(mangaId: string): string {
        return `${KEMONO_DOMAIN}${mangaId}`;
    }

    getArtists = async (): Promise<Artist[]> => {
        const url = `${KEMONO_DOMAIN}/api/v1/creators.txt`
    ...
    async getMangaDetails(mangaId: string): Promise<SourceManga> {
        ...
        return App.createSourceManga({
            ...
            mangaInfo: App.createMangaInfo({
                ...
                author: name,
                artist: name,
                image: `${IMG_KEMONO_DOMAIN}/${artist.service}/${artist.id}`,
                desc: "",
                status: "",
            }),
        })
    ...
    async getChapters(mangaId: string): Promise<Chapter[]> {
        ...
        const $ = await this.DOMHTML(`${KEMONO_DOMAIN}${mangaId}`);

        const total = parseInt($('small').text().match(/\d+/g)?.pop() || '0');

        // Loop to make concurrent requests for chapters
        while (offset < total) {
            requests.push(this.GetJSON(`${KEMONO_DOMAIN}/api/v1${mangaId}?o=${offset}`));
            offset += batchSize;
        }
    ...
    async getChapterDetails(
        ...
        const url = `${KEMONO_DOMAIN}/api/v1${mangaId}/post/${chapterId}`
        const request = App.createRequest({
            url: url,
            method: 'GET',
            headers: {
        ...
        pages.push(`${KEMONO_DOMAIN}/data${json.file.path}`)

        const attachments = json.attachments
        for (const attachment of attachments) {
            pages.push(`${KEMONO_DOMAIN}/data${attachment.path}`)
        }
