...
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.Kemono = exports.KemonoInfo = void 0;
const types_1 = require("@paperback/types");
const KemonoParser_1 = require("./KemonoParser");
...
class Kemono {
    constructor(cheerio) {
            ...
            interceptor: {
                interceptRequest: async (request) => {
                    request.headers = {
                        ...(request.headers ?? {}),
                        ...{
                            referer: `${constant_1.KEMONO_DOMAIN}/`,
        ...
        this.getArtists = async () => {
            const url = `${constant_1.KEMONO_DOMAIN}/api/v1/creators.txt`;
            ...
            return (0, KemonoParser_1.parseArtists)(json);
        };
        this.artists = this.getArtists();
    }
    getMangaShareUrl(mangaId) {
        return `${constant_1.KEMONO_DOMAIN}${mangaId}`;
    }
    async GetJSON(url) {
        const request = App.createRequest({
            url: url,
    ...
    async getMangaDetails(mangaId) {
        ...
        const artist = (0, KemonoParser_1.getArtistById)(artists, id);
        ...
        return App.createSourceManga({
            ...
            mangaInfo: App.createMangaInfo({
                ...
                author: name,
                artist: name,
                image: `${constant_1.IMG_KEMONO_DOMAIN}/${artist.service}/${artist.id}`,
                desc: "",
                status: "",
            }),
        });
    ...
    async getChapters(mangaId) {
        ...
        const $ = await this.DOMHTML(`${constant_1.KEMONO_DOMAIN}${mangaId}`);
        const total = parseInt($('small').text().match(/\d+/g)?.pop() || '0');
        // Loop to make concurrent requests for chapters
        while (offset < total) {
            requests.push(this.GetJSON(`${constant_1.KEMONO_DOMAIN}/api/v1${mangaId}?o=${offset}`));
            offset += batchSize;
        }
        ...
        for (const response of responses) {
            const batchChapters = (0, KemonoParser_1.parseChapterList)(response);
    ...
    async getChapterDetails(mangaId, chapterId) {
        const pages = [];
        const url = `${constant_1.KEMONO_DOMAIN}/api/v1${mangaId}/post/${chapterId}`;
        const request = App.createRequest({
            url: url,
            method: 'GET',
            headers: {
        ...
        const response = await this.requestManager.schedule(request, 1);
        const json = (typeof response.data === 'string') ? JSON.parse(response.data) : response.data;
        pages.push(`${constant_1.KEMONO_DOMAIN}/data${json.file.path}`);
        const attachments = json.attachments;
        for (const attachment of attachments) {
            pages.push(`${constant_1.KEMONO_DOMAIN}/data${attachment.path}`);
        }
    ...
    async getHomePageSections(sectionCallback) {
        ...
        for (const section of sections) {
            ...
            const response = (0, KemonoParser_1.searchArtists)(artists, searchQuery.searchName, searchQuery.orderBy, searchQuery.orderDirection, searchQuery.filterService, page, pageSize);
            const items = [];
...
exports.Kemono = Kemono;

},{"./KemonoParser":63,"./constant":64,"./external/search.json":65,"@paperback/types":61}],63:[function(require,module,exports){
"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
